...
export function Videos() {
  ...
  const fetchVideos = async () => {
    try {
      const response = await axios.get('http://localhost:3000/ext/videos');
  ...
  const handleUpload = async (e: Event) => {
    ...
    try {
      await axios.post('http://localhost:3000/ext/videos', formData, {
        headers: {
  ...
  const handleDelete = async (id: string) => {
    try {
      await axios.delete(`http://localhost:3000/ext/videos/${id}`);
  ...
  const handleToggleActive = async (id: string, active: boolean) => {
    try {
      ...
      if (video) {
        await axios.put(`http://localhost:3000/ext/videos/${id}`, {
          ...video,
          active: !active
        });
        fetchVideos();
    ...
    <div class="container mx-auto p-4">
      ...
      <div class="mb-8">
        ...
        {videos.value.length > 0 && (
          <div>
            <video 
              src={`http://localhost:3000/ext/stream/${videos.value[currentVideoIndex.value].id}`} 
              controls 
