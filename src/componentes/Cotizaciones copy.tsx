...
export function Cotizacion() {
  ...
  const fetchComunicados = async () => {
    try {
      const response = await axios.get('http://192.168.194.31/ext/comunicados');
  ...
  const fetchCotizaciones = async () => {
    try {
      const response = await axios.get('http://192.168.194.31/ext/cotizaciones');
  ...
  const handleComunicadoSubmit = async (e: Event) => {
    ...
    try {
      if (editingComunicado) {
        await axios.put(`http://192.168.194.31/ext/comunicados/${editingComunicado}`, comunicadoForm);
      } else {
        await axios.post('http://192.168.194.31/ext/comunicados', { ...comunicadoForm, usuarioId: 'user123' });
      }
  ...
  const handleCotizacionSubmit = async (e: Event) => {
    ...
    try {
      if (editingCotizacion) {
        await axios.put(`http://192.168.194.31/ext/cotizaciones/${editingCotizacion}`, cotizacionForm);
      } else {
        await axios.post('http://192.168.194.31/ext/cotizaciones', { ...cotizacionForm, usuarioId: 'user123' });
      }
  ...
  const deleteComunicado = async (id: string) => {
    try {
      await axios.delete(`http://192.168.194.31/ext/comunicados/${id}`);
      fetchComunicados();
    } catch (error) {
  ...
  const deleteCotizacion = async (id: string) => {
    try {
      await axios.delete(`http://192.168.194.31/ext/cotizaciones/${id}`);
      fetchCotizaciones();
    } catch (error) {
