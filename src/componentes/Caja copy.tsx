...
export function Caja({handleLogout}: {handleLogout: () => void}) {
  ...
  //         // Esperar a que se establezca el puntoAtencion antes de hacer las llamadas
  //         await obtenerProximaFicha(storedPuntoAtencion.id);
  //         await obtenerFichasPendientes();
  //       } else {
  ...
  // const obtenerProximaFicha = async () => {
  //   if (!puntoAtencion) {
  //     console.error('No hay punto de atención definido');
  //     return;
  //   }
  //   try {
  //     const response = await apiCall<Ficha | null>(`/api/proxima-ficha/${puntoAtencion.id}`);
  //     console.log('Respuesta de próxima ficha:', response);
  ...
  const llamarFicha = async () => {
    console.log('estado cola',estadoCola);
    ...
    try {
      ...
      console.log('response llamar fichar',response.ficha);
      setEstadoCola(prevEstado => ({
        ...prevEstado,
